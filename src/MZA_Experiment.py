...
class MZA_Experiment(DynSystem_Data, Train_Methodology):
    ...
    def __init__(self,args):
        ...
        if str(type(args)) != "<class 'dict'>":
            ...
            self.exp_name      = "sl{sl}_obs{numobs}_bs{bs}_attblks{attblocks}_atthds{nheads}_tr{tr}_ph{ph}_nhd{nhd}_{beta_VAE}_{info}".format(nhd = args.hidden_dim, beta_VAE = args.beta_VAE, sl = args.seq_len, numobs = args.num_obs, bs=args.bs, attblocks = args.nattblocks, nheads = args.nheads, tr = args.ntransients, ph = args.pred_horizon, lambda_stateloss = args.lambda_stateloss, info=args.info)
...
###########################################################################################################################
    ...
    def lr_lambda(opt, epoch):
        ...
        if 0 <= epoch < 500:
            ...
            return 1e-5
        elif 2000 <= epoch < 3000:
            return 5e-6
        elif 3000 <= epoch < 4000: 
            return 1e-6
        elif 4000 <= epoch < 6000: 
            return 5e-7
        else:
            return 1e-7
...
###########################################################################################################################
    def plot_learning_curves(self):
        ...
        #State Loss
        plt.figure()
        plt.semilogy(df['epoch'], df['Uncertainty'], label="Test State Evolution Loss")
        plt.legend()
        plt.xlabel("Epochs")
        plt.savefig(self.exp_dir+'/'+self.exp_name+"/out_log/StateLoss.png", dpi = 256, facecolor = 'w', bbox_inches='tight')
        plt.close()
