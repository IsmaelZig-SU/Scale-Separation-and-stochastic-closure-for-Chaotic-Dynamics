...
################################################################################################################################################
    def energy(self, u, v):
        ...
        k = 0.5 * (torch.mean(u**2, dim=-1) + torch.mean(v**2, dim=-1))
        return k
    ...
    def train_test_loss(self, mode = "Train", dataloader = None):
        ...
        for Phi_seq, Phi_nn_ph in dataloader:
            ...
            x_nn_ph , Phi_nn_hat_ph_nolatentevol, _, _ = self.model.autoencoder(Phi_nn_ph)
            ...
            true_kinetic_energy = self.energy(Phi_nn_ph[..., :Phi_nn_ph.shape[-1]//2],Phi_nn_ph[..., Phi_nn_ph.shape[-1]//2:])
            tot_kinetic_energy = self.energy(Phi_nn_hat_ph[..., :Phi_nn_hat_ph.shape[-1]//2], Phi_nn_hat_ph[..., Phi_nn_hat_ph.shape[-1]//2:])
            ...
            Energy_Loss = mseLoss(true_kinetic_energy, tot_kinetic_energy) 

       
            loss = TransEvo_Loss + self.lambda_stateloss*StateEvo_Loss + 100*Autoencoder_Loss + Energy_Loss
...
################################################################################################################################################
    ...
    def training_loop(self):
        ...
        for ix_epoch in range(self.load_epoch, self.load_epoch + self.nepochs):
            ...
            train_Ldict = self.train_test_loss(mode = "Train", dataloader = None)
            test_Ldict  = self.train_test_loss(mode = "Test", dataloader = self.test_dataloader)
